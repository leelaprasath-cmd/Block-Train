...
import { LASER_FRAGMENT_SHADER, LASER_VERTEX_SHADER } from './ThreeUILaserShaders';
...
export interface ThreeUILaserProps {
...
export const THREEUI_LASER_DEFAULTS = {
...
export const ThreeUILaser: React.FC<ThreeUILaserProps> = ({
  className = '',
  style,
  interactive = true,
  ...props
  ...
  const optionsRef = useRef({ ...THREEUI_LASER_DEFAULTS, ...props, interactive });
  optionsRef.current = { ...THREEUI_LASER_DEFAULTS, ...props, interactive };
  ...
  useEffect(() => {
    ...
    try {
      vertexShader = compileShader(gl, gl.VERTEX_SHADER, LASER_VERTEX_SHADER);
      fragmentShader = compileShader(gl, gl.FRAGMENT_SHADER, LASER_FRAGMENT_SHADER);
      ...
      console.warn('ThreeUI Laser initialization warning:', err);
      return undefined;
    }
    ...
    const draw = (now: number) => {
      ...
      const variant =
        options.variant in VARIANT_INDEX ? options.variant : THREEUI_LASER_DEFAULTS.variant;
      ...
      gl.uniform1f(
        uniforms.time,
        reducedMotion ? 2.75 : (now - startedAt) * 0.001 * clamp(options.speed, 0, 3)
      );
